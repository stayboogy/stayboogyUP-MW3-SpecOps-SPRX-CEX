...
void stayboogy(uint64_t arg)
{
	...
	for (;;)
	{
			// sleep 10 seconds for game to boot
			// thread repeats every 10 seconds
			// the button hold duration interval (roughly) for Manly | Godly
			sleep(10000);

			// if Dpad Left is held for (roughly) 10 seconds
			if  (GetKey()==(PAD_LEFT))
			{
				Manly();
				{
					// sleep for 30 seconds so Player has enough time to die
					sleep(30000);
				}
			}

			// do the cheats with no user interaction
			// hud menus are gay af
			// secret cheater
			UAV();
			Perks();
			Loaded();
			...
			// if Dpad Right is held for (roughly) 10 seconds
			//if (GetKey()==(PAD_RIGHT))
			//{
			//	Godly();
			//}
